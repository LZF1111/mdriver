...
# Windows 默认 stdout=GBK，遇到 † / ‘ / 　 等非 ASCII 会报 UnicodeEncodeError。
# 强制 stdout/stderr = UTF-8，Node 端也会用 UTF-8 解析。
try:
    ...
    for i in range(n_pages):
        ...
        if pytess and ocr_pages < OCR_MAX_PAGES and ocr_chars < OCR_MAX_CHARS and len(txt.strip()) < 40:
            try:
                ...
                # 同时保存扫描页栅格图（供前端预览）—— 一律存 JPEG（quality=85），
                # 因为某些 VLM 上游 (SiliconFlow/Kimi-VL) 对 PyMuPDF 保存的 PNG 做 verify
                # 会报 "Verify image file failed: verify must be called directly after open"。
                # JPEG 没有 alpha / pHYs / iCCP 等花活，验证总能通过。
                if IMG_OUT_DIR and SAVE_SCAN_PAGES:
                    try:
                        scan_path = os.path.join(IMG_OUT_DIR, f"scan_p{i+1:03d}.jpg")
                        # 优先用已 frombytes 出来的 im（pytess 路径），否则现做一个
                        try:
                            _im_for_save = im  # noqa: F821 — 上面 OCR 路径若成功就有 im
                        except Exception:
                            from PIL import Image as _PILImg
                            _im_for_save = _PILImg.frombytes("RGB", [pix.width, pix.height], pix.samples)
                        _im_for_save.save(scan_path, "JPEG", quality=85, optimize=True)
                        saved_images.append({
                            'path': scan_path, 'page': i+1, 'index': 0,
                            'w': pix.width, 'h': pix.height, 'ext': 'jpg',
